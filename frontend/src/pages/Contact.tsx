...
export const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Custom validation check for all fields
    if (
      !formData.name.trim() ||
      !formData.phone.trim() ||
      !formData.email.trim() ||
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to submit contact query.');
    } finally {
...
- *Travel Date*: ${formData.travel_date.trim()}
  ...
  return (
    <div className="relative min-h-screen bg-slate-50/50 pb-20">
      ...
      <PageHero
        ...
        disableShadow={true}
        buttonText="Send Inquiries Below"
      />
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 text-xs">
                  <div>
                    <label className="block font-bold text-slate-700 mb-1.5 uppercase">Travel Date</label>
                    <input
                      type="date"
                      name="travel_date"
                      required
                ...
                <div className="pt-2 flex gap-3">
                  <button
                    ...
                    className="flex-1 rounded-xl bg-gradient-premium py-3 text-sm font-semibold text-white shadow-md hover:shadow-lg hover:scale-102 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSubmitting ? 'Submitting...' : 'Submit Quotation Inquiry'}
                  </button>
                  <button
                    type="button"
                    onClick={triggerWhatsAppContact}
